import React from "react";
import "./Footer.css";
import {
  FaYoutube,
  FaFacebookSquare,
  FaTwitter,
  FaInstagram,
} from "react-icons/fa";
import gurantee from "../../assets/images/gurantee.png";

const Footer = () => {
  return (
    <>
      {/* <div classNameName="footer">
        <div classNameName="inner-footer">
          <div classNameName="footer-items">
            <h3>Online Shopping</h3>
            <ul>
              <a href="#">
                <li>Men</li>
              </a>
              <a href="#">
                <li>Women</li>
              </a>
              <a href="#">
                <li>Kids</li>
              </a>
              <a href="#">
                <li>Beauty</li>
              </a>
              <a href="">
                <li>Gift-cards</li>
              </a>
              <a href="">
                <li>Myntra Insider</li>
              </a>
            </ul>
          </div>

          <div classNameName="footer-items">
            <h3>Useful Links</h3>
            <ul>
              <a href="#">
                <li>Contact Us</li>
              </a>
              <a href="#">
                <li>FAQ</li>
              </a>
              <a href="#">
                <li>T&C</li>
              </a>
              <a href="#">
                <li>Terms Of Use</li>
              </a>
              <a href="#">
                <li>Shipping</li>
              </a>
              <a href="#">
                <li>Returns</li>
              </a>
              <a href="#">
                <li>Blog</li>
              </a>
              <a href="#">
                <li>Careers</li>
              </a>
              <a href="#">
                <li>SiteMap</li>
              </a>
              <a href="#">
                <li>Cancellation</li>
              </a>
            </ul>
          </div>

          <div classNameName="footer-items">
            <h3>Experience myntra app on mobile</h3>
            <ul classNameName="app-store">
              <a href="#">
                <img
                  src="https://constant.myntassets.com/web/assets/img/80cc455a-92d2-4b5c-a038-7da0d92af33f1539674178924-google_play.png"
                  alt=""
                />
              </a>
              <a href="">
                <img
                  src="https://constant.myntassets.com/web/assets/img/bc5e11ad-0250-420a-ac71-115a57ca35d51539674178941-apple_store.png"
                  alt=""
                />
              </a>
            </ul>
            <h3>Keep In Touch</h3>
            <div classNameName="border1"></div>
            <ul classNameName="social-icons">
              <a href="#">
                <span>
                  <FaFacebookSquare />
                </span>
                <span>
                  <FaTwitter />
                </span>
                <span>
                  <FaYoutube />
                </span>
                <span>
                  <FaInstagram />
                </span>
              </a>
            </ul>
          </div>

          <div classNameName="footer-items ">
            <ul>
              <a href="#">
                <img src={gurantee} alt="" />
              </a>
            </ul>
          </div>
        </div>

        <div classNameName="footer-bottom">
          <a>POPULAR SEARCHES</a>
          <br /><br />
          <a href="">Shorts | </a>  
          <a href="">Jeans | </a>
          <a href="">Party Wear | </a>
          <a href="">Jackets | </a>
          <a href="">Trousers | </a>
          <a href="">Jumpsuits | </a>
          <a href="">Innerwear | </a>
          <a href="">Track Pants | </a>
          <a href="">Ripped jeans | </a>
          <a href="">T-shirts | </a>
          <a href="">Men Jeans | </a>
          <a href="">Gym Wear | </a>
          <a href="">Men Trunks | </a>
          <a href="">Shirts | </a>
          <a href="">Pants | </a>
          <a href="">Kurtas | </a>
          <a href="">Kurtis | </a>
          <a href="">Underwear | </a>
          <a href="">Bra | </a>
          <a href="">Panties | </a>
          <a href="">Lingrie | </a>
          <a href="">T-shirts | </a>
          <a href="">Men Jeans | </a>
          <a href="">Gym Wear | </a>
          <a href="">Men Trunks | </a>
          <a href="">Shirts | </a>
          <a href="">Pants | </a>
          <a href="">Jeans | </a>
          <a href="">Party Wear | </a>
          <a href="">Jackets | </a>
          <a href="">Trousers | </a>
          <a href="">Jumpsuits | </a>
          <a href="">Innerwear | </a>
          <a href="">Track Pants | </a>
          <a href="">Ripped jeans | </a>
          <a href="">T-shirts | </a>
          <a href="">Men Jeans | </a>
          <a href="">Gym Wear | </a>
          <a href="">Men Trunks | </a>
          <a href="">Shirts | </a>
          </div>
          <br />
          <div classNameName="contact-section">
              <a href="" classNameName="left">In case of any concern <span>Contact us</span></a>
              <a href="" classNameName="right">© 2022 www.myntra.com. All rights reserved.</a>
          </div>
        </div> */}
        <div style={{position: "relative"}}>
        <div id="web-footerMount">
            <div style={{position:"relative",zIndex:"5" ,padding:"30px 0px 40px 0px",background:"#FAFBFC"}} data-reactroot=""
                data-reactid="1" data-react-checksum="473104049">
                <footer className="desktop-footerContainer" data-reactid="2">
                    <div data-reactid="3">
                        <div className="index-seoContainer" data-reactid="4">
                            <div className="index-priceContainer" data-reactid="5">
                                <h2 className="index-title" data-reactid="6">
                                   
                                </h2>
                                <table style={{marginTop: "10px"}} data-reactid="10">
                                    <thead data-reactid="11">
                                        <tr data-reactid="12">
                                            <th data-reactid="13">Shirts For Men &amp; Women</th>
                                            <th data-reactid="14">PRICE (RS)</th>
                                        </tr>
                                    </thead>
                                    <tbody data-reactid="15"></tbody>
                                </table>
                                <div className="index-dateTitle" data-reactid="16">
                                    
                                </div>
                            </div>
                            <h2 className="index-title" data-reactid="19">
                               
                            </h2>
                            <div className="index-descContainer" data-reactid="23">
                                <p>
                                    <strong>
                                        <h3>
                                            FABULOUS SHIRTS FOR THE FASHION-FORWARD
                                        </h3>
                                    </strong>
                                </p>
                                <p>
                                    Gone are the days when shirts were considered apt only for a formal environment.
                                    Aware of this change in fashion choices, Myntra brings you a line of smart shirts
                                    online for fashion-forward women, men, boys and girls. Be it regular work wear, a
                                    boardroom
                                    presentation, a semi-formal party, a dinner date or a trip to your favourite holiday
                                    destinations; Myntra has the perfect option for you. At Myntra, you are offered a
                                    wide variety of shirts with price options that are
                                    quite affordable. While you are checking out our collection of shirts online, you
                                    can also look for a sharp pair of trousers, <a href="/jeans"
                                        className="seolink">jeans</a> and suits
                                    to stock up on ensemble options in one go.
                                </p>
                                <p>
                                    <strong>
                                        THE SHIRTS ONLINE SHOPPING GUIDE
                                    </strong>
                                </p>
                                <p>
                                    At Myntra, our exclusive collection of shirt brands features a wide number of
                                    patterns, colours, cuts and styles. In terms of fabrics, our collection of shirts
                                    online include great options in cotton, poly-cotton, crepe, georgette, linen,
                                    chiffon, polyester,
                                    satin, silk and more. You can even buy shirts in the kind of fit that you would like
                                    such as regular, boxy, skinny, tailored or slim-fit versions.
                                </p>
                                <p>
                                    We now progress towards the most important stage of your shirt shopping journey -
                                    the selection of right patterns that suit you the most. Checks, solids and stripes
                                    are the most popular options, with men and women opting for one of them as their
                                    oft-repeated
                                    personal style. You can figure out yours as well. You can also look at faded,
                                    printed and self-design shirts online which offer you many popular ensemble
                                    alternatives.
                                </p>
                                <p>
                                    If you thought that the popular button-down collars were the only options for your
                                    neckline, think again. Browse through Myntra’s range of Peter Pan collars, cutaway
                                    and wingtip collars which offer smart alternatives. The mandarin collar shirts with
                                    their
                                    eastern appeal are versatile enough to be worn for both formal and casual occasions.
                                    For a preppy look, opt for hooded collars with drawstrings. Club collars, as the
                                    name suggests, are great for creating a standout
                                    image at parties. Or you can simply go collarless with our collection of cool casual
                                    shirts online.
                                </p>
                                <p>
                                    <strong>
                                        UNLIMITED STYLE OPTIONS WITH SMART SHIRTS ONLINE
                                    </strong>
                                </p>
                                <p>
                                    Shirts are quite versatile when worn the right way. Here are some style suggestions
                                    on how to wear shirts for different occasions:
                                </p>
                                <p>
                                    Plaid or checked shirts for men can be worn in a number of ways to achieve both
                                    casual and formal looks. Simply pair them up with jeans and loafers for a summer
                                    outing. Or try a laid-back winter look; choose a layered option with
                                    <a href="/tshirts" className="seolink">T-shirts</a> and jackets. A pair of light blue or
                                    faded jeans and trainers, along with a messenger bag offer, give your look a neat
                                    finish.
                                </p>
                                <p>
                                    Denim shirts for men are classNameics which shall always remain trendy. Make your own
                                    casual statement with a long denim shirt with folded up sleeves, along with dark
                                    coloured jeans. Pair the outfit with dark boots and top it off with a skull cap. You
                                    can
                                    even wear a cardigan on top of your <a href="/men-denim-shirts"
                                        className="seolink">denim shirt</a> for chilly weather.
                                </p>
                                <p>
                                    Ladies out there, confused on how to get ready in a snap for an impromptu outing
                                    with friends? Choose your favourite shirt and wear them untucked with a pair of
                                    skinny jeans. Wear round-toe flats and jazz it up with chunky bangles or
                                    <a href="/bracelet" className="seolink">bracelets</a>. Carry your valuables in your
                                    favourite tote and you are done. This outfit provides you with the best of both
                                    worlds when it comes to comfort and style.
                                </p>
                                <p>
                                    Take your pick from some of the stylish formal shirt brands to make a smart
                                    statement at work. One style option you can opt for is mandarin neck collars that
                                    are paired with your favourite pencil skirt and strappy block heels. Or opt for a
                                    bright solid
                                    shirt and wear it with dark trousers and pumps. Women can also go for matte-finish
                                    makeup for a sharp look. To give your formal look a sharp finish, choose studs from
                                    Myntra’s stunning collection of earrings online.
                                </p>
                                <p>
                                    Myntra’s collection of shirts online also include cute options for kids. You can
                                    select any casual, formal or party shirts for your little boys and girls. Go all out
                                    with their style preferences and pick their favourite from different varieties of
                                    prints,
                                    checks, stripes and patterns.
                                </p>
                                <p>
                                    <strong>
                                        <h3>
                                            BUY SHIRTS ONLINE WITH EASE
                                        </h3>
                                    </strong>
                                </p>
                                <p>
                                    If there is one place where you can buy shirts online with complete convenience, it
                                    is at Myntra. Myntra brings to you simplified browsing and payment processes that
                                    ensure an enjoyable shopping experience without any hindrances. The fantastic array
                                    of
                                    shirt brands Myntra offers feature multiple style options which cater to several
                                    different needs, so you find shirts that perfectly suit your personal style. While
                                    you are doing so, you can also check out at other interesting
                                    varieties from our clothing range including T-shirts, dresses, skirts, <a
                                        href="/jackets" className="seolink">jackets</a>, sweaters, sweatshirts and more.
                                </p>
                                <div itemprop="video" itemscope itemtype="http://schema.org/VideoObject">
                                    <h3>VIDEO: <span itemprop="name">How to Tuck in a Shirt Properly - Men's Style
                                            Guide</span></h3><br/>
                                    <meta itemprop="duration" content="PT1M17S" />
                                    <meta itemprop="thumbnailUrl"
                                        content="https://i.ytimg.com/vi/hGZ244SCZ-Y/hqdefault.jpg?sqp=-oaymwEZCPYBEIoBSFXyq4qpAwsIARUAAIhCGAFwAQ==&rs=AOn4CLCh3PJ14W7qjRfpg9kYWXKswroVfA" />
                                    <meta itemprop="contentURL" content="https://www.youtube.com/watch?v=hGZ244SCZ-Y" />
                                    <meta itemprop="embedURL" content="https://www.youtube.com/embed/hGZ244SCZ-Y" />
                                    <meta itemprop="uploadDate" content="2014-12-2T08:00:00+08:00" />
                                    <meta itemprop="expires" content="2050-07-27T08:00:00+08:00" />
                                    <meta itemprop="height" content="315" />
                                    <meta itemprop="width" content="315" />
                                    <meta itemprop="interactionCount" content="38251" />
                                    
                                        {/* <iframe width="560" height="315" src="https://www.youtube.com/embed/hGZ244SCZ-Y"
                                            frameborder="0"
                                            allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"
                                            allowfullscreen></iframe > */}
                                    
                                    <span itemprop="description">
                                        <p>At Myntra, our exclusive collection of shirt brands features a wide number of
                                            patterns, colours, cuts and styles. </p>
                                    </span>
                                </div>
                                
                            </div>
                        </div>
                        <div className="desktop-genericInfo" data-reactid="24">
                            <div className="desktop-shopLinks" data-reactid="25">
                                <p className="desktop-gInfoTitle" data-reactid="26"><a href="/?src=onlineShopping"
                                        data-reactid="27"> ONLINE SHOPPING </a></p>
                                <a href="/shop/men" data-reactid="28">
                                    Men
                                </a>
                                <a href="/shop/women" data-reactid="32">
                                  Women
                                </a>
                                <a href="/shop/kids" data-reactid="36">
                                    Kids
                                </a>
                                <a href="/shop/home-living" data-reactid="40">
                                   Home &amp; Living
                                </a>
                                <a href="/personal-care" data-reactid="44">
                                  Beauty
                                </a><a href="/giftcard" data-reactid="48">Gift Cards</a>
                                <a href="/myntrainsider?cache=false" data-reactid="49">
                                    Myntra Insider
                                    <span className="desktop-superscript" data-reactid="51"> New </span>
                                </a>
                            </div>
                            <div className="desktop-usefulLinks" data-reactid="52">
                                <p className="desktop-gInfoTitle" data-reactid="53"> USEFUL LINKS </p>
                                <a href="/contactus" data-reactid="54">
                                    Contact Us
                                </a>
                                <a href="/faqs" data-reactid="58">
                                   FAQ
                                </a>
                                <a href="/tac" data-reactid="62">
                                   T &amp; C
                                </a>
                                <a href="/termsofuse" data-reactid="66">
                                    Terms Of Use
                                </a>
                                <a href="/my/orders" data-reactid="70">
                                    Track Orders
                                </a>
                                <a href="/faqs#shipping" data-reactid="74">
                                    Shipping
                                </a>
                                <a href="/faqs#cancel" data-reactid="78">
                                   Cancellation
                                </a>
                                <a href="/faqs#returns" data-reactid="82">
                                   Returns
                                </a>
                                <a href="/security/whitehat" data-reactid="86">
                                    Whitehat
                                </a>
                                <a href="http://blog.myntra.com/" data-reactid="90">
                                    Blog
                                </a>
                                <a href="https://careers.myntra.com" data-reactid="94">
                                    Careers
                                </a>
                                <a href="/privacypolicy" data-reactid="98">
                                    Privacy Policy
                                </a>
                                <a href="/sitemap" data-reactid="102">
                                    Site Map
                                </a>
                                
                            </div>
                            <div className="desktop-appExperience" data-reactid="106">
                                <p className="desktop-gInfoTitle" data-reactid="107"> EXPERIENCE MYNTRA APP ON MOBILE </p>
                                <div className="desktop-downLinkContainer" data-reactid="108">
                                    <img src="https://constant.myntassets.com/web/assets/img/80cc455a-92d2-4b5c-a038-7da0d92af33f1539674178924-google_play.png" alt="" style={{height:"42px"}}/>
                                        <img src="https://constant.myntassets.com/web/assets/img/80cc455a-92d2-4b5c-a038-7da0d92af33f1539674178924-google_play.png" alt="" style={{height:"42px"}}/>  
                                </div>
                                <div className="desktop-keepInTouch" data-reactid="113"> KEEP IN TOUCH </div>
                                
                                <FaFacebookSquare className="desktop-facebook " style={{height:"20px", color: "#696e79"}} />
                                   
                               
                                
                                <FaTwitter className="desktop-twitter" style={{height:"20px", color: "#696e79"}}  />
                                    
                              
                                      <FaYoutube className="desktop-youtube" style={{height:"20px", color: "#696e79"}}  />
                                   
                               
                                <FaInstagram className="desktop-instagra" style={{height:"20px", color: "#696e79"}}  />
                                    
                            </div>
                            <div className="desktop-promises" data-reactid="122">
                                <div className="desktop-section" data-reactid="123">
                                    <div className="desktop-original" data-reactid="124">
                                    <img src="https://constant.myntassets.com/web/assets/img/6c3306ca-1efa-4a27-8769-3b69d16948741574602902452-original.png" alt="" style={{height:"40px"}} />
                                    </div>
                                    <div data-reactid="126"><strong data-reactid="127">100% ORIGINAL </strong>
                                        guarantee for all products at myntra.com
                                       
                                    </div>
                                </div>
                                <div className="desktop-section" data-reactid="129">
                                    <div className="desktop-return" data-reactid="130">
                                    <img src="https://constant.myntassets.com/web/assets/img/ef05d6ec-950a-4d01-bbfa-e8e5af80ffe31574602902427-30days.png" alt="" style={{height:"40px"}} />
                                    </div>
                                    <div data-reactid="132"><strong data-reactid="133">Return within 30days </strong>
                                        of receiving your order
                                        
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="desktop-popularSearch" data-reactid="135" style={{marginTop: "80px"}}>
                            <div data-reactid="136">
                                <hr data-reactid="137" />
                                <div className="desktop-pSearchTitle" data-reactid="138"> POPULAR SEARCHES </div>
                                <div className="desktop-pSearchlinks" data-reactid="139">
                                    
                                    <a href="/shorts" data-reactid="141">
                                        Shorts
                                    </a>
                                    <a href="/jeans" data-reactid="145">
                                       Jeans
                                    </a>
                                    <a href="/party-wear" data-reactid="149">
                                       Party Wear
                                    </a>
                                    <a href="/jackets" data-reactid="153">
                                        Jackets
                                    </a>
                                    <a href="/trousers" data-reactid="157">
                                       Trousers 
                                    </a>
                                    <a href="/jumpsuit" data-reactid="161">
                                        JumpSuit
                                    </a>
                                    <a href="/innerwear" data-reactid="165">
                                       Innerwear
                                    </a>
                                    <a href="/track-pants" data-reactid="169">
                                       Track Pants
                                    </a>
                                    <a href="/ripped-jeans" data-reactid="173">
                                        Ripped Jeans
                                    </a>
                                    <a href="/coats" data-reactid="177">
                                        Coats
                                    </a>
                                    <a href="/men-tshirts" data-reactid="181">
                                        Men Tshirts
                                    </a>
                                    <a href="/capris" data-reactid="185">
                                        Capris
                                    </a>
                                    <a href="/biker-jackets" data-reactid="189">
                                        Biker Jackets
                                    </a>
                                    <a href="/shrug" data-reactid="193">
                                        Shrug
                                    </a>
                                    <a href="/tshirts" data-reactid="197">
                                       Tshirts
                                    </a>
                                    <a href="/men-kurtas" data-reactid="201">
                                            Men Kurtas
                                    </a>
                                    <a href="/kurta-pyjama" data-reactid="205">
                                       Kurta Pyjama
                                    </a>
                                    <a href="/sweaters" data-reactid="209">
                                       Sweaters
                                    </a>
                                    
                                    <a href="/thongs" data-reactid="293">
                                        
                                        Thongs
                                        
                                    </a>
                                    <a href="/sports-wear" data-reactid="297">
                                      
                                        Sportswear
                                        
                                    </a>
                                    <a href="/gym-wear" data-reactid="301">
                                        
                                        Gym Wear
                                       
                                    </a>
                                    <a href="/sweatshirts" data-reactid="305">
                                       
                                        Sweatshirts
                                        
                                    </a>
                                    <a href="/men-tshirts" data-reactid="309">
                                       
                                      Men T Shirts
                                        
                                    </a>
                                    <a href="/men-trousers" data-reactid="313">
                                       
                                        Men Trousers
                                        
                                    </a>
                                    <a href="/men-jackets" data-reactid="317">
                                       
                                        Men Jackets
                                        
                                    </a>
                                    <a href="/men-sweaters" data-reactid="321">
                                        
                                        Men Sweaters
                                       
                                    </a>
                                    <a href="/men-shrugs" data-reactid="325">
                                       
                                      Men Shrugs
                                        
                                    </a>
                                    <a href="/men-pants" data-reactid="329">
                                      
                                      Men Pants
                                        
                                    </a>
                                    <a href="/men-jeans" data-reactid="333">
                                        
                                    Men Jeans
                                        
                                    </a>
                                    <a href="/men-track-pants" data-reactid="337">
                                      
                                        Men Track Pants
                                       
                                    </a>
                                    <a href="/men-tshirts" data-reactid="181">
                                        Men Tshirts
                                    </a>
                                    <a href="/capris" data-reactid="185">
                                        Capris
                                    </a>
                                    <a href="/biker-jackets" data-reactid="189">
                                        Biker Jackets
                                    </a>
                                    <a href="/shrug" data-reactid="193">
                                        Shrug
                                    </a>
                                    <a href="/tshirts" data-reactid="197">
                                       Tshirts
                                    </a>
                                    <a href="/men-kurtas" data-reactid="201">
                                            Men Kurtas
                                    </a>
                                    <a href="/kurta-pyjama" data-reactid="205">
                                       Kurta Pyjama
                                    </a>
                                    <a href="/sweaters" data-reactid="209">
                                       Sweaters
                                    </a>
                                    <a href="/men-tshirts" data-reactid="181">
                                        Men Tshirts
                                    </a>
                                    <a href="/capris" data-reactid="185">
                                        Capris
                                    </a>
                                    <a href="/biker-jackets" data-reactid="189">
                                        Biker Jackets
                                    </a>
                                    <a href="/shrug" data-reactid="193">
                                        Shrug
                                    </a>
                                    <a href="/tshirts" data-reactid="197">
                                       Tshirts
                                    </a>
                                    <a href="/men-kurtas" data-reactid="201">
                                            Men Kurtas
                                    </a>
                                    <a href="/kurta-pyjama" data-reactid="205">
                                       Kurta Pyjama
                                    </a>
                                    <a href="/sweaters" data-reactid="209">
                                       Sweaters
                                    </a>
                                </div>
                                
                            </div>
                        </div>
                        <div className="desktop-fInfoSection" data-reactid="343">
                            <div className="desktop-contact" data-reactid="344">
                                In case of any concern,
                               <a href="/contactus" data-reactid="346"> Contact Us </a>
                            </div>
                            <div className="desktop-copywrite" data-reactid="347">
                                2022
                                www.myntra.com. All rights reserved.
                                
                            </div>
                        </div>
                    </div>
                </footer>
            </div>
        </div>
    </div>
        
    </>
  );
};

export default Footer;
